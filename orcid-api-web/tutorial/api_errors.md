...
|400| One of the provided parameters is invalid, or, the provided token/code is invalid or expired | This happens when you provide invalid parameters (client id, secret or redirect uri) on authorization code exchange, check the provided parameters|--|
|400| Invalid authorization code: [code] | This happens when you exchange an invalid authorization code, check the `code` parameter|--|
...
|400|	Invalid scope: /webhook	|Your credentials are not authorized to create webhooks. Webhooks are available only to premium members, if you are a premium member contact https://support.orcid.org/ to correct this problem|--|
