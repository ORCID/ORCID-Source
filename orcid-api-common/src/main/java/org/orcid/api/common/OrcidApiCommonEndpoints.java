...
public class OrcidApiCommonEndpoints {
    ...
    public Response obtainOauth2TokenPost(@HeaderParam("Authorization") @DefaultValue(StringUtils.EMPTY) String authorization,
        ...
        boolean hasBasicAuthorization = StringUtils.isNotBlank(authorization);
        if(!hasBasicAuthorization) {
            if(clientId == null || clientId.length() > 50 || StringUtils.isBlank(clientId)) {
        ...
        try {
            ...
            return Response.status(Response.Status.GATEWAY_TIMEOUT)
                    .entity(e.getMessage())
                    .header(Features.OAUTH_AUTHORIZATION_CODE_EXCHANGE.name(), "ON")
                    .build();
        }
