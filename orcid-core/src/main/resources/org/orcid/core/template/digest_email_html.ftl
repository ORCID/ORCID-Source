...
<html>
    ...
    <body>
        <div style="padding: 20px; padding-top: 0px; font-family: arial, helvetica, sans-serif; font-size: 15px; color: #666666; width: 800px;">
            ...
            <p> 
            	<div><img src="${baseUri}/static/img/request.png">&nbsp;<@emailMacros.msg 'email.institutional_connection.1' />${(notification.idpName)!}<@emailMacros.msg 'email.institutional_connection.2' /><a href="${baseUri}/inbox/encrypted/${notification.encryptedPutCode}/action"><@emailMacros.msg "email.institutional_connection.here" /></a><@emailMacros.msg 'email.institutional_connection.3' />${(notification.source.sourceName.content)!sourceId}<@emailMacros.msg 'email.institutional_connection.4' /> <#if notification.createdDate??>(${notification.createdDate.year?c}-<#if notification.createdDate.month?string?length == 1>0${notification.createdDate.month?c}<#else>${notification.createdDate.month?c}</#if>-<#if notification.createdDate.day?string?length == 1>0${notification.createdDate.day?c}<#else>${notification.createdDate.day?c}</#if>)</#if></div>
            </p>
            <#else>
