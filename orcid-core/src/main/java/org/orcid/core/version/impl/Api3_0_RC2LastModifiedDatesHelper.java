...
public class Api3_0_RC2LastModifiedDatesHelper {
    ...
    public static void calculateLastModified(PeerReviews peerReviews) {
        if (peerReviews != null && peerReviews.getPeerReviewGroup() != null && !peerReviews.getPeerReviewGroup().isEmpty()) {
            LastModifiedDate peerReviewsLatest = null;
            for (PeerReviewGroup group : peerReviews.getPeerReviewGroup()) {
                LastModifiedDate groupReviewsLatest = null;
                for (PeerReviewDuplicateGroup duplicateGroup : group.getPeerReviewGroup()) {
                    LastModifiedDate duplicateGroupReviewsLatest = null;
                    for (PeerReviewSummary summary : duplicateGroup.getPeerReviewSummary()) {
                        LastModifiedDate lastModified = summary.getLastModifiedDate();
                        if (lastModified != null && lastModified.after(peerReviewsLatest)) {
                            peerReviewsLatest = lastModified;
                        }
                        if (lastModified != null && lastModified.after(groupReviewsLatest)) {
                            groupReviewsLatest = lastModified;
                        }
                        if (lastModified != null && lastModified.after(duplicateGroupReviewsLatest)) {
                            duplicateGroupReviewsLatest = lastModified;
                        }
                    }
                    duplicateGroup.setLastModifiedDate(duplicateGroupReviewsLatest);
                }
                group.setLastModifiedDate(groupReviewsLatest);
            }
            peerReviews.setLastModifiedDate(peerReviewsLatest);
        }
    }
