...
public class PIDResolverService {
     ...
     *         a flag indicating success and a flag indicating if resolution was
     *         attempted (i.e. there is a resolver that can handle the type)
     */
    ...
    /**
     * Returns a Work populated with metadata attached to the PID NOTE: may only
     * be semi-populated. Ensure this is the API type name, not the DB type
     * name.
     * 
     * @param type
     *            the api type name
     * @param value
     *            the url value
     * @return a work containing all the available metadata we could find or
     *         NULL if none found.
     */
