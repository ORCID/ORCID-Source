...
import org.orcid.jaxb.model.v3.release.record.ExternalIdentifiersContainer;
...
public class ActivitiesGroup {
    ...
    @Deprecated
    /**
     * This method is only used by tests to confirm accuracy of
     * ActivitiesGroupGenerator and should not be used in production
     * 
     * @param activity
     * @return
     */
    public boolean belongsToGroup(GroupableActivity activity) {
        ...
        if (groupKeys == null || groupKeys.isEmpty()) {
            if (activity.getExternalIdentifiers() == null || activity.getExternalIdentifiers().getExternalIdentifier() == null
                    || activity.getExternalIdentifiers().getExternalIdentifier().isEmpty()) {
                // Check if the activity dont have grouping keys
                // If the activity doesn't have any external identifier,
                // check if the activity is in the group
                if (activities.contains(activity))
                    return true;
                else
                    return false;
            } else {
                // If any of the activities pass the grouping validation,
                // the activity must belong to other group
                for (GroupAble extId : activity.getExternalIdentifiers().getExternalIdentifier()) {
                    if (extId.isGroupAble())
                        return false;
                }

                // If none of the activities pass the groupings validation,
                // so, lets check if the group actually contains the
                // activity
                if (activities.contains(activity))
                    return true;
                else
        ...
        if (container != null) {
            ...
            for (GroupAble extId : extIds) {
                ...
                if (extId.isGroupAble()) {
                    // If any of the keys already exists on this group,
                    // return true
