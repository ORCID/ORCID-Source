...
public interface FuzzyDateMapperV3 {
    ...
    // ========================================================================
    // Type Coercion Helpers (JAXB Wrappers <-> Integer)
    // ========================================================================
    ...
    default Integer map(Year year) {
        return (year == null || year.getValue() == null) ? null : Integer.valueOf(year.getValue().trim());
    }

    default Integer map(Month month) {
        return (month == null || month.getValue() == null) ? null : Integer.valueOf(month.getValue().trim());
    }

    default Integer map(Day day) {
        return (day == null || day.getValue() == null) ? null : Integer.valueOf(day.getValue().trim());
    }
}