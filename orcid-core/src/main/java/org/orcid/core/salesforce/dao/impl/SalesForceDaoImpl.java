...
public class SalesForceDaoImpl implements SalesForceDao, InitializingBean {
    ...
    private JSONObject retrieveMembersObject(String accessToken, String accountId) {
        ...
        if(Features.SF_ENABLE_OPP_ORG_RECORD_TYPES.isActive()) {
            query.append(
                "SELECT Account.Id, Account.Consortium_Lead__c, Account.OwnerId, Account.Name, Account.Public_Display_Name__c, Account.Website, Account.BillingCountry, Account.Research_Community__c, Account.Consortia_Member__c, ");
        } else {
            query.append(
        ...
        query.append(
                "Account.Public_Display_Description__c, Account.Logo_Description__c, Account.Public_Display_Email__c, Account.Last_membership_start_date__c, Account.Last_membership_end_date__c from Account WHERE Active_Member__c=TRUE AND (RecordTypeId NOT IN (" + ids + ") OR (RecordTypeId IN (" + ids + ")))");
        if (accountId != null) {
            validateSalesForceId(accountId);
