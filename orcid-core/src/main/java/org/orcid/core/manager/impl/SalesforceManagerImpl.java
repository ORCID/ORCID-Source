...
public class SalesforceManagerImpl implements SalesforceManager {
	...
	public MemberDetails retrieveMemberDetails(String memberSlug) {
		...
		try {
			...
			// Set parent org name
			String parentOrgName = memberDetailsJsonObject.has("parentOrgName")
					? memberDetailsJsonObject.getString("parentOrgName")
					: null;
			...
			if (memberDetailsJsonObject.has("member")) {
				...
				// Set parent slug
				String consortiumLeadId = memberJsonObject.has("Consortium_Lead__c")
						? memberJsonObject.getString("Consortium_Lead__c")
						: null;
			...
			if (memberDetailsJsonObject.has("consortiumOpportunities")) {
				...
				if (consortiumOpportunities != null && consortiumOpportunities.length() > 0) {
					List<Opportunity> opportunityList = salesForceAdapter
							.createOpportunitiesList(consortiumOpportunities);
