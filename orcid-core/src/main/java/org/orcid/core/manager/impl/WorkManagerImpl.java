...
public class WorkManagerImpl implements WorkManager {
	 ...
	 * @param workInfo
	 * */
	private void loadWorkSourceInfo(ProfileEntity sourceEntity,
			WorkInfoEntity workInfo) {
		...
		if (OrcidType.CLIENT.equals(sourceEntity.getOrcidType())) {
			...
			if (Visibility.PUBLIC.equals(sourceNameVisibility)) {
				...
				String name = sourceEntity.getGivenNames()
	 ...
	 * @param workInfo
	 * */
	private void loadWorkContributors(ProfileWorkEntity profileWorkEntity,
			WorkInfoEntity workInfo) {
		WorkContributors workContributors = jpa2JaxbAdapter
				.getWorkContributors(profileWorkEntity);
	...
	private void loadWorkInfo(WorkEntity workEntity, WorkInfoEntity workInfo) {
		...
		if (workEntity.getPublicationDate() != null) {
			if (workEntity.getPublicationDate().getDay() != null)
				workInfo.setPublicationDay(workEntity.getPublicationDate()
						.getDay());
			if (workEntity.getPublicationDate().getMonth() != null)
				workInfo.setPublicationMonth(workEntity.getPublicationDate()
						.getMonth());
			if (workEntity.getPublicationDate().getYear() != null)
				workInfo.setPublicationYear(workEntity.getPublicationDate()
						.getYear());
		}
		...
		workInfo.setTranslatedTitleLanguageCode(workEntity
	 ...
	 * @param workInfo
	 * */
	private void loadWorkExternalIdentifiers(WorkEntity workEntity,
			WorkInfoEntity workInfo) {
		workInfo.setExternalIdentifiers(jpa2JaxbAdapter
				.getWorkExternalIdentifiers(workEntity));
	}
