...
public class RecordStatusDao {
	...
	public RecordStatusEntity get(String orcid) {
		Query query = entityManager.createNativeQuery("SELECT * FROM record_status WHERE orcid = :orcid",
				RecordStatusEntity.class);
	...
	public boolean updateStatus(String orcid, AvailableBroker broker, Integer status) {
		Query query = entityManager.createNativeQuery(
				"UPDATE record_status SET " + broker + " = :status, last_modified = now() WHERE orcid = :orcid");
		query.setParameter("orcid", orcid);
	...
	public boolean updateStatus(String orcid, AvailableBroker broker) {
		Query query = entityManager.createNativeQuery("UPDATE record_status SET " + broker + " = (" + broker
