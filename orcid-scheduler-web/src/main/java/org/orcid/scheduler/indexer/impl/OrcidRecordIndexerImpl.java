...
public class OrcidRecordIndexerImpl implements OrcidRecordIndexer {
    ...
    private boolean processProfilesForIndexing(IndexingStatus status) {
        ...
        try {
            ...
            if (lastSlackNotification == null || System.currentTimeMillis() > (lastSlackNotification.getTime() + (slackIntervalMinutes * 60 * 1000))) {
                String message = String.format(
                        "Unable to fetch records with indexing status: %s, this causes that SOLR and S3 might be falling behind. For troubleshooting please refere to https://github.com/ORCID/ORCID-Internal/wiki/Problems-with-record-indexing-in-the-scheduler",
                        status);
        ...
        for (String orcid : orcidsForIndexing) {
            ...
            if (!connectionIssue) {
                try {
                    ...
                    // Send a slack notification every 'slackIntervalMinutes'
                    // minutes
                    if (lastSlackNotification == null || System.currentTimeMillis() > (lastSlackNotification.getTime() + (slackIntervalMinutes * 60 * 1000))) {
        ...
        // returns true if there wasn't a connection issue and the indexing
        // batch size was reached meaning a new batch should be processed if
        // batch count was not reached
