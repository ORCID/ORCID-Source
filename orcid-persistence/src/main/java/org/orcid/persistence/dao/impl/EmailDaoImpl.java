...
public class EmailDaoImpl extends GenericDaoImpl<EmailEntity, String> implements EmailDao {
    ...
    @Override
    @Transactional
    public void revertUserOBODetails(List<String> ids) {
        Query query = entityManager.createNativeQuery("UPDATE email SET assertion_origin_source_id = NULL where email_hash IN :ids");
        query.setParameter("ids", ids);
        query.executeUpdate();
    }
