...
public class ProfileDaoImpl extends GenericDaoImpl<ProfileEntity, String>
	...
	public List<ProfileEntity> retrieveSelectableSponsors() {
		return (List<ProfileEntity>) entityManager
				.createQuery(
						"from ProfileEntity where isSelectableSponsor=true order by vocativeName")
				.getResultList();
	}
	...
	public List<String> findOrcidsByName(String name) {
		TypedQuery<String> query = entityManager
				.createQuery(
						"select id from ProfileEntity where lower(givenNames) like lower(:name || '%') or lower"
								+ "(familyName) like lower(:name || '%') or lower(vocativeName) like lower(:name || "
								+ "'%') or lower(creditName) like lower(:name || '%')",
						String.class);
	...
	@SuppressWarnings("unchecked")
	public List<String> findOrcidsByIndexingStatus(
			IndexingStatus indexingStatus, int maxResults) {
		return findOrcidsByIndexingStatus(indexingStatus, maxResults,
	...
	public List<String> findOrcidsByIndexingStatus(
		...
		return findOrcidsByIndexingStatus(indexingStatuses, maxResults,
				orcidsToExclude);
	}
	...
	@Override
	public List<String> findOrcidsByIndexingStatus(
			Collection<IndexingStatus> indexingStatuses, int maxResults,
			Collection<String> orcidsToExclude) {
		...
		query.setParameter("indexingStatus",
				IndexingStatus.getNames(indexingStatuses));
		if (!orcidsToExclude.isEmpty()) {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<String> findUnclaimedNotIndexedAfterWaitPeriod(
			int waitPeriodDays, int maxDaysBack, int maxResults,
			Collection<String> orcidsToExclude) {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<String> findEmailsUnverfiedDays(int daysUnverified,
			int maxResults, EmailEventType ev) {
		// @formatter:off
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<String> findUnclaimedNeedingReminder(int remindAfterDays,
			int maxResults, Collection<String> orcidsToExclude) {
		StringBuilder builder = new StringBuilder(
		...
		builder.append(" ORDER BY p.last_modified");
		Query query = entityManager.createNativeQuery(builder.toString());
		query.setParameter("profileEventType",
				ProfileEventType.CLAIM_REMINDER_SENT.name());
		if (!orcidsToExclude.isEmpty()) {
			query.setParameter("orcidsToExclude", orcidsToExclude);
		}
		query.setMaxResults(maxResults);
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<String> findByEventTypes(int maxResults,
			List<ProfileEventType> pets, Collection<String> orcidsToExclude,
			boolean not) {
		/*
	...
	public List<String> findOrcidsNeedingEmailMigration(int maxResults) {
		TypedQuery<String> query = entityManager
				.createQuery(
						"select p.id from ProfileEntity p where email is not null and orcidType != 'CLIENT'",
						String.class);
		query.setMaxResults(maxResults);
		return query.getResultList();
	}
	...
	public boolean orcidExists(String orcid) {
		TypedQuery<Long> query = entityManager.createQuery(
				"select count(pe.id) from ProfileEntity pe where pe.id=:orcid",
				Long.class);
	...
	public void remove(String giverOrcid, String receiverOrcid) {
		...
		if (profileEntity != null) {
			if (profileEntity.getGivenPermissionTo() != null) {
				...
				for (GivenPermissionToEntity givenPermissionToEntity : profileEntity
						.getGivenPermissionTo()) {
					if (!receiverOrcid.equals(givenPermissionToEntity
							.getReceiver().getId())) {
	...
	private void removeChildren(String orcid,
			Collection<? extends BaseEntity<?>> entities) {
		removeChildren(orcid, entities, "profile.id");
	}

	private void removeChildren(String orcid,
			Collection<? extends BaseEntity<?>> entities, String orcidPath) {
		if (entities != null && !entities.isEmpty()) {
			entityManager
					.createQuery(
							"delete from "
									+ entities.iterator().next().getClass()
											.getName() + " where " + orcidPath
									+ " = :orcid").setParameter("orcid", orcid)
					.executeUpdate();
			entities.clear();
		}
	}
	...
	public boolean exists(String orcid) {
		TypedQuery<Long> query = entityManager.createQuery(
				"select count(p.id) from ProfileEntity p where p.id = :orcid",
				Long.class);
		query.setParameter("orcid", orcid);
		Long result = query.getSingleResult();
		return (result != null && result > 0);
	...
	public IndexingStatus retrieveIndexingStatus(String orcid) {
		TypedQuery<IndexingStatus> query = entityManager
				.createQuery(
						"select indexingStatus from ProfileEntity where orcid = :orcid",
	...
	public Long getConfirmedProfileCount() {
		TypedQuery<Long> query = entityManager
				.createQuery(
						"select count(pe) from ProfileEntity pe where pe.completedDate is not null",
						Long.class);
		return query.getSingleResult();
	}
	...
	public boolean updateProfile(ProfileEntity profile) {
		...
		query.setParameter("iso2_country",
				iso2Country != null ? iso2Country.value() : null);
		query.setParameter("biography_visibility",
				StringUtils.upperCase(profile.getBiographyVisibility().value()));
		query.setParameter("keywords_visibility",
				StringUtils.upperCase(profile.getKeywordsVisibility().value()));
		query.setParameter("researcher_urls_visibility", StringUtils
				.upperCase(profile.getResearcherUrlsVisibility().value()));
		query.setParameter("other_names_visibility", StringUtils
				.upperCase(profile.getOtherNamesVisibility().value()));
		query.setParameter("credit_name_visibility", StringUtils
				.upperCase(profile.getCreditNameVisibility().value()));
		query.setParameter("profile_address_visibility", StringUtils
				.upperCase(profile.getProfileAddressVisibility().value()));
	...
	public Date retrieveLastModifiedDate(String orcid) {
		TypedQuery<Date> query = entityManager.createQuery(
				"select lastModified from ProfileEntity where orcid = :orcid",
				Date.class);
		query.setParameter("orcid", orcid);
		return query.getSingleResult();
	}
	...
	@Override
	@Transactional
	public void updateLastModifiedDateAndIndexingStatusWithoutResult(
			String orcid, Date lastModified, IndexingStatus indexingStatus) {
		Query query = entityManager
				.createNativeQuery("update profile set last_modified = :lastModified, indexing_status = :indexingStatus where orcid = :orcid ");
		query.setParameter("orcid", orcid);
	...
	private void updateWebhookProfileLastUpdate(String orcid) {
		Query query = entityManager
				.createNativeQuery("update webhook set profile_last_modified = (select last_modified from profile where orcid = :orcid ) "
						+ "where orcid = :orcid ");
		query.setParameter("orcid", orcid);
		query.executeUpdate();
	}
	...
	public void updateLastModifiedDateAndIndexingStatus(String orcid) {
		Query updateQuery = entityManager
				.createQuery("update ProfileEntity set lastModified = now(), indexingStatus = 'PENDING' where orcid = :orcid");
	...
	public OrcidType retrieveOrcidType(String orcid) {
		TypedQuery<OrcidType> query = entityManager.createQuery(
				"select orcidType from ProfileEntity where orcid = :orcid",
				OrcidType.class);
		query.setParameter("orcid", orcid);
	...
	public List<Object[]> findInfoForDecryptionAnalysis() {
		Query query = entityManager
				.createQuery("select id, encryptedSecurityAnswer from ProfileEntity");
		@SuppressWarnings("unchecked")
	...
	public Locale retrieveLocale(String orcid) {
		TypedQuery<Locale> query = entityManager.createQuery(
				"select locale from ProfileEntity where orcid = :orcid",
				Locale.class);
		query.setParameter("orcid", orcid);
		return query.getSingleResult();
	}
	...
	public void updateLocale(String orcid, Locale locale) {
		Query updateQuery = entityManager
				.createQuery("update ProfileEntity set lastModified = now(), locale = :locale where orcid = :orcid");
		updateQuery.setParameter("orcid", orcid);
	...
	public String retrievePrimaryAccountOrcid(String deprecatedOrcid) {
		Query query = entityManager
				.createNativeQuery("select primary_record from profile where orcid = :orcid");
		query.setParameter("orcid", deprecatedOrcid);
	...
	public void updateEncryptedPassword(String orcid, String encryptedPassword) {
		Query updateQuery = entityManager
				.createQuery("update ProfileEntity set lastModified = now(), encryptedPassword = :encryptedPassword where orcid = :orcid");
		updateQuery.setParameter("orcid", orcid);
	...
	@Override
	@Transactional
	public void updateSecurityQuestion(String orcid,
			Integer securityQuestionId, String encryptedSecurityAnswer) {
		Query updateQuery = entityManager
		...
		updateQuery.setParameter("encryptedSecurityAnswer",
	...
	@Override
	@Transactional
	public void updatePreferences(String orcid,
			boolean sendChangeNotifications, boolean sendOrcidNews,
			boolean sendMemberUpdateRequests,
			Visibility activitiesVisibilityDefault,
		...
		updateQuery.setParameter("orcid", orcid);
		updateQuery.setParameter("sendChangeNotifications",
				sendChangeNotifications);
		updateQuery.setParameter("sendOrcidNews", sendOrcidNews);
		updateQuery.setParameter("sendMemberUpdateRequests",
				sendMemberUpdateRequests);
		updateQuery.setParameter("activitiesVisibilityDefault",
				activitiesVisibilityDefault);
		updateQuery.setParameter("enableDeveloperTools", enableDeveloperTools);
		updateQuery.setParameter("sendEmailFrequencyDays",
	...
	@Override
	@Transactional
	public void updateCountry(String orcid, Iso3166Country iso2Country,
			Visibility profileAddressVisibility) {
		Query updateQuery = entityManager
		...
		updateQuery.setParameter("orcid", orcid);
		updateQuery.setParameter("iso2Country",
				iso2Country != null ? iso2Country.value() : null);
	...
	public Iso3166Country getCountry(String orcid) {
		TypedQuery<Iso3166Country> query = entityManager.createQuery(
				"select iso2_country from ProfileEntity where orcid = :orcid",
	...
	public void updateBiography(String orcid, String biography,
		...
		updateQuery.setParameter("visibility", visibility == null ? null
				: StringUtils.upperCase(visibility.value()));
		updateQuery.executeUpdate();
	}

	@Override
	@Transactional
	public void updateNames(String orcid, String givenNames, String familyName,
			String creditName, Visibility creditNameVisibility) {
		Query updateQuery = entityManager
		...
		updateQuery.setParameter(
				"creditNameVisibility",
				creditNameVisibility == null ? null : StringUtils
						.upperCase(creditNameVisibility.value()));
		updateQuery.executeUpdate();
	}
	...
	public List<ProfileEntity> findProfilesByOrcidType(OrcidType type) {
		Query query = entityManager
				.createQuery("from ProfileEntity where profile_deactivation_date=NULL and orcidType=:type");
	...
	public boolean updateDeveloperTools(String orcid, boolean enabled) {
		Query query = entityManager
				.createQuery("update ProfileEntity set enableDeveloperTools=:enabled, lastModified=now() where orcid=:orcid");
		if (enabled)
			query = entityManager
	...
	@Override
	@Transactional
	public boolean updateResearcherUrlsVisibility(String orcid,
			Visibility visibility) {
		Query query = entityManager
	...
	public boolean getClaimedStatus(String orcid) {
		Query query = entityManager
				.createNativeQuery("select claimed from profile where orcid=:orcid");
		query.setParameter("orcid", orcid);
	...
	public ClientType getClientType(String orcid) {
		TypedQuery<ClientType> query = entityManager.createQuery(
				"select clientType from ClientDetailsEntity where id = :orcid",
				ClientType.class);
		query.setParameter("orcid", orcid);
	...
	public MemberType getGroupType(String orcid) {
		TypedQuery<MemberType> query = entityManager.createQuery(
				"select groupType from ProfileEntity where orcid = :orcid",
				MemberType.class);
		query.setParameter("orcid", orcid);
	...
	public boolean isLocked(String orcid) {
		TypedQuery<Boolean> query = entityManager.createQuery(
				"select recordLocked from ProfileEntity where orcid = :orcid",
				Boolean.class);
		query.setParameter("orcid", orcid);
	...
	public boolean isDeactivated(String orcid) {
		TypedQuery<Date> query = entityManager
				.createQuery(
						"select deactivationDate from ProfileEntity where orcid = :orcid",
						Date.class);
		query.setParameter("orcid", orcid);
