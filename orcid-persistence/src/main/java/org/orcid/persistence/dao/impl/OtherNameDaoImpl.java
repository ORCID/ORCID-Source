...
public class OtherNameDaoImpl extends GenericDaoImpl<OtherNameEntity, Long> implements OtherNameDao {
    ...
    public void revertUserOBODetails(List<BigInteger> ids) {
        Query query = entityManager.createNativeQuery("UPDATE other_name SET assertion_origin_source_id = NULL where other_name_id IN :ids");
        query.setParameter("ids", ids);
        query.executeUpdate();
    }
    ...
    public boolean updateVisibility(String orcid, Visibility visibility) {
        Query query = entityManager.createNativeQuery("UPDATE other_name SET visibility = :visibility WHERE orcid = :orcid");
        query.setParameter("orcid", orcid);
