...
<script type="text/ng-template" id="affiliation-ext-id-popover-ng2-template">
    ...
    <a *ngIf="extID?.externalIdentifierId && extID?.url?.value" href="{{extID.url.value}}" class="truncate-anchor inline" rel="noopener noreferrer" target="orcid.blank" (mouseenter)="showAffiliationExtIdPopOver(putCode)" (mouseleave)="hideAffiliationExtIdPopOver(putCode)">{{extID.externalIdentifierId.value}}</a> 
        ...
        <div class="popover-help-container">
            <div class="popover bottom" [ngClass]="{'block' : displayAffiliationExtIdPopOver[putCode] == true}">
                ...
                <div class="popover-content">
                    <a href="{{extID.url.value}}" rel="noopener noreferrer" target="orcid.blank" class="ng-binding">{{extID.url.value}}</a>
                </div>
            </div>
        </div>
    </div>             
