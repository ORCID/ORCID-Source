<script type="text/ng-template" id="notification-body-ng2-template">
    <div class="notification">
        <div class="notifications-inner">
                ...
                <div *ngIf="TOGGLZ_VERBOSE_NOTIFICATIONS; else defaultNotifications">
                    <div *ngIf="(addedList.length > 0 || updatedList.length > 0 || deletedList.length > 0 || unknownList.length > 0); else noItemNotification">
                        <p><strong>{{notification?.source?.sourceName?.content}}</strong>&nbsp;<a
                                    ...
                                    class="glyphicon glyphicon-question-sign oauth-question-sign"></a> <@orcid.msg 'notifications.updated.label.1'/>
                            {{notification?.amendedSection | replaceSeparatorWithSpace |
                            titlecase}} <@orcid.msg 'notifications.updated.label.2'/></p>
                        ...
                        <div *ngIf="addedList.length > 0">
                            ...
                            <ul *ngFor="let element of addedList">
                                <li>
                                    <ng-container
                                            *ngIf="element.additionalInfo && element.additionalInfo['org_name'] != undefined">
                                        <i>{{element.additionalInfo['org_name']}} </i>
                                    </ng-container>
                                    <ng-container *ngIf="element.itemName != null">{{element.itemName}}</ng-container>
                                    <ng-container
                                            *ngIf="element.itemName == null && element.additionalInfo && element.additionalInfo['group_name'] != undefined">
                                        {{element.additionalInfo['group_name']}}
                                    </ng-container>
                    ...
                    <div *ngIf="elementsModifiedCount >= MAX_ELEMENTS_TO_SHOW"><@orcid.msg 'notifications.updated.showing' />
                        {{MAX_ELEMENTS_TO_SHOW}} {{notification?.amendedSection | replaceSeparatorWithSpace |
                        titlecase}} <@orcid.msg 'notifications.updated.out_of' />
                        {{elementsModifiedCount}}<@orcid.msg 'notifications.updated.please_check.1' />
                        {{notification?.amendedSection | replaceSeparatorWithSpace |
                        titlecase}}<@orcid.msg 'notifications.updated.please_check.2' /> <a
                                href="{{getBaseUri()}}/my-orcid"
                                target="_blank"><@orcid.msg 'notifications.updated.please_check.3' /></a></div>
                    <div class="topBuffer">
            ...
            <!--CUSTOM-->
            <div *ngIf="notification.notificationType=='ADMINISTRATIVE' || notification.notificationType=='CUSTOM' || notification.notificationType=='SERVICE_ANNOUNCEMENT' || notification.notificationType=='TIP'"
                 [innerHTML]="notification.bodyHtml | extractContentFromBody">
            </div>
            ...
            <div *ngIf="notification.notificationType=='INSTITUTIONAL_CONNECTION'">
                <p *ngIf="notification?.authorizationUrl">
                    <@orcid.msg 'email.institutional_connection.1' />
                    {{notification?.idpName}} <@orcid.msg 'email.institutional_connection.2' /><a
                            href="{{getBaseUri()}}/inbox/{{notification?.putCode}}/action?target={{encodedUrl}}"
                            target="email.institutional_connection.here"><@orcid.msg 'email.institutional_connection.here' /></a><@orcid.msg 'email.institutional_connection.3' />
                    {{notification?.source?.sourceName?.content}} <@orcid.msg 'email.institutional_connection.4' />
                </p>
                <p *ngIf="!notification?.authorizationUrl">
                    <@orcid.msg 'email.institutional_connection.disabled.1' />
                    {{notification?.idpName}}<@orcid.msg 'email.institutional_connection.disabled.2' />
                </p>
            ...
            <div *ngIf="notification.notificationType=='PERMISSION'">
                ...
                <div *ngIf="notification?.authorizationUrl">
                    ...
                    <div class="topBuffer pull-right">
                        <button *ngIf="!notification?.archivedDate" (click)="archive(notification.putCode)"
                                class="btn btn-white-no-border cancel-left"><@orcid.msg 'notifications.archivewithoutgranting' /></button>
                        <a class="btn btn-primary"
                           href="{{getBaseUri()}}/inbox/{{notification.putCode}}/action?target={{encodedUrl}}"
                           target="notifications.grant_permissions"><span
                                    class="glyphicons cloud-upload"></span> <@orcid.msg 'notifications.grant_permissions'/>
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="topBuffer">
                <small><@orcid.msg 'notifications.agreement_advice'/> <a
                            href="<@orcid.msg 'common.kb_uri_default'/>360006972953"
                            target="notifications.learn_more"><@orcid.msg 'notifications.learn_more'/></a>
                </small>
            </div>
        </div>
    </div>
