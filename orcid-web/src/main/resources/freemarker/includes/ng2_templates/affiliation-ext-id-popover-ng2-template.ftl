...
<script type="text/ng-template" id="affiliation-ext-id-popover-ng2-template">
    ...
    <a *ngIf="extID.externalIdentifierId" href="{{extID.url.value}}" class="truncate-anchor inline" target="orcid.blank" (mouseenter)="showAffiliationExtIdPopOver(putCode)" (mouseleave)="hideAffiliationExtIdPopOver(putCode)">{{extID.externalIdentifierId.value}}</a> 
