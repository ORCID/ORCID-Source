...
<@protected classes=['manage'] nav="settings">
	...
	<div class="col-md-9">
		...
		<p>
			<strong></strong><a href="<@orcid.msg 'manage_delegators.learn_more.link.url' />" ><@orcid.msg 'manage_delegators.learn_more.link.text' /></a></strong>&nbsp;<@orcid.msg 'manage_delegators.learn_more.text' />
		</p>
			...
			<table class="table table-bordered settings-table normal-width" ng-show="delegators.delegationDetails" ng-cloak>
				...
				<tbody>
					<tr ng-repeat="delegationDetails in delegators.delegationDetails | orderBy:sort.column:sort.descending">
						<td width="35%"><a href="<@orcid.rootPath '/switch-user?username='/>{{delegationDetails.delegateSummary.orcidIdentifier.path}}" >{{delegationDetails.delegateSummary.creditName.content}}</a></td>
