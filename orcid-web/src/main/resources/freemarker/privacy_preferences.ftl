...
<div class="popover-frame">
	...
	<form id="privacy-pref-form" class="form-horizontal" action="<@spring.url '/account/update-privacy-preferences'/>" method="post" autocomplete="off">
	    <label class="privacy-toggle-lbl">${springMacroRequestContext.getMessage("privacy_preferences.labelDefaultprivacyfornewWorks")}</label>
        ...
        <div class="relative">
            <button id="privacy-pref-submit-changes" class="btn btn-primary">${springMacroRequestContext.getMessage("freemarker.btnsavechanges")}</button>
            <button id="privacy-pref-clear-changes" class="btn close-parent-popover" type="reset">${springMacroRequestContext.getMessage("freemarker.btncancel")}</button>
        </div>
    </form>
</div>
</@base>
