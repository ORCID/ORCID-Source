...
export class AffiliationFormComponent implements AfterViewInit, OnDestroy, OnInit {
    ...
    urlPreCheck() {
        ...
        if (regexpURL.test(this.editAffiliation.url.value)) {
          return;
        } else if (regexpURL.test("http://" + this.editAffiliation.url.value)) {
          this.editAffiliation.url.value = "http://" + this.editAffiliation.url.value;
        }
    }
