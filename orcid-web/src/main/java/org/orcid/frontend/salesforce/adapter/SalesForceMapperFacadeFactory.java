...
public class SalesForceMapperFacadeFactory implements FactoryBean<MapperFacade> {
    ...
    private void registerMemberMap(MapperFactory mapperFactory) {
    ...
    private void registerIntegrationMap(MapperFactory mapperFactory) {
        ...
        classMap.customize(new CustomMapper<Integration, JSONObject>() {
            ...
            public void mapBtoA(JSONObject b, Integration a, MappingContext context) {
                ...
                // Filter the stage
                try {
                    String integrationStage = b.getString("Integration_Stage__c");
                    switch(integrationStage) {
                    case "In Development":
                    case "Complete":
                    case "Certified Service Provider":
                    case "None Planned":
                        a.setStage(integrationStage);
                        break;
                    }
                } catch(Exception e) {
                    // Nothing to handle
