...
public class AdminController extends BaseController {
	...
	public void setOrcidClientGroupManager(
			OrcidClientGroupManager orcidClientGroupManager) {
	...
	public ModelAndView getDeprecatedProfilesPage() {
		...
		OrcidProfile profile = orcidProfileManager.retrieveOrcidProfile(
	...
	public @ResponseBody ProfileDeprecationRequest deprecateProfile(
			@RequestParam("deprecated") String deprecatedOrcid,
			@RequestParam("primary") String primaryOrcid) {
		ProfileDeprecationRequest result = new ProfileDeprecationRequest();
		// Check for errors
		if (deprecatedOrcid == null
				|| !OrcidStringUtils.isValidOrcid(deprecatedOrcid)) {
			result.getErrors().add(
					getMessage(
							"admin.profile_deprecation.errors.invalid_orcid",
							deprecatedOrcid));
		} else if (primaryOrcid == null
				|| !OrcidStringUtils.isValidOrcid(primaryOrcid)) {
			result.getErrors().add(
					getMessage(
							"admin.profile_deprecation.errors.invalid_orcid",
							primaryOrcid));
			...
			result.getErrors()
					.add(getMessage("admin.profile_deprecation.errors.deprecated_equals_primary"));
		} else {
			try {
				boolean wasDeprecated = adminManager.deprecateProfile(result,
						deprecatedOrcid, primaryOrcid);
				if (wasDeprecated) {
					ProfileEntity deprecated = profileEntityCacheManager
							.retrieve(deprecatedOrcid);
					ProfileEntity primary = profileEntityCacheManager
							.retrieve(primaryOrcid);
				...
				result.getErrors()
						.add(getMessage(
								"admin.profile_deprecation.errors.internal_error",
								deprecatedOrcid));
			}
		}
	 ...
	 *         error message.
	 * */
	@RequestMapping(value = { "/deprecate-profile/check-orcid.json",
			"/deactivate-profile/check-orcid.json" }, method = RequestMethod.GET)
	public @ResponseBody ProfileDetails checkOrcidToDeprecate(
			@RequestParam("orcid") String orcid) {
		ProfileDetails profileDetails = new ProfileDetails();
		try {
			OrcidProfile profile = orcidProfileManager.retrieveOrcidProfile(
					orcid, LoadOptions.BIO_ONLY);
			if (profile != null) {
				if (profile.getOrcidDeprecated() != null
						&& profile.getOrcidDeprecated().getDate() != null) {
					profileDetails
							.getErrors()
							.add(getMessage(
									"admin.profile_deprecation.errors.already_deprecated",
									orcid));
				} else if (profile.isDeactivated()) {
					profileDetails
							.getErrors()
							.add(getMessage(
									"admin.profile_deactivation.errors.already_deactivated",
									orcid));
				} else {
					...
					if (profile != null
						...
						if (profile.getOrcidBio().getPersonalDetails()
								.getFamilyName() != null) {
							profileDetails.setFamilyName(profile.getOrcidBio()
									.getPersonalDetails().getFamilyName()
									.getContent());
							hasName = true;
						}
						if (profile.getOrcidBio().getPersonalDetails()
								.getGivenNames() != null) {
							profileDetails.setGivenNames(profile.getOrcidBio()
									.getPersonalDetails().getGivenNames()
									.getContent());
							hasName = true;
						}
						...
						if (!hasName) {
							profileDetails.setGivenNames(profile.getOrcidBio()
									.getPersonalDetails().getCreditName()
									.getContent());
						}
					}
					if (profile.getOrcidBio().getContactDetails() != null
							&& profile.getOrcidBio().getContactDetails()
									.getEmail() != null) {
						for (Email email : profile.getOrcidBio()
				...
				profileDetails
						.getErrors()
						.add(getMessage(
								"admin.profile_deprecation.errors.inexisting_orcid",
								orcid));
			}
	...
	public @ResponseBody ProfileDetails deactivateOrcidAccount(
			@RequestParam("orcid") String orcid) {
		OrcidProfile toDeactivate = orcidProfileManager
		...
		if (toDeactivate == null)
			...
			result.getErrors()
					.add(getMessage("admin.profile_deactivation.errors.already_deactivated"));
		else if (toDeactivate.getOrcidDeprecated() != null)
			result.getErrors().add(
					getMessage("admin.errors.deprecated_account"));
	...
	public @ResponseBody ProfileDetails reactivateOrcidAccount(
			@RequestParam("orcid") String orcid) {
		OrcidProfile toReactivate = orcidProfileManager
				.retrieveOrcidProfile(orcid);
		ProfileDetails result = new ProfileDetails();
		if (toReactivate == null)
			...
			result.getErrors()
					.add(getMessage("admin.profile_reactivation.errors.already_active"));
	...
	public @ResponseBody List<ProfileDetails> findIdByEmail(
			@RequestParam("csvEmails") String csvEmails) {
	...
	public @ResponseBody String generateRandomString() {
		return RandomStringUtils.random(RANDOM_STRING_LENGTH,
				OrcidPasswordConstants.getEntirePasswordCharsRange());
	}
	...
	public @ResponseBody String resetPassword(HttpServletRequest request,
			@RequestBody AdminChangePassword form) {
		String orcidOrEmail = form.getOrcidOrEmail();
		String password = form.getPassword();
		String orcid = null;
		if (StringUtils.isNotBlank(password)
				&& password
						.matches(OrcidPasswordConstants.ORCID_PASSWORD_REGEX)) {
			...
			if (OrcidType.ADMIN.equals(currentProfile.getType())) {
				...
				if (StringUtils.isNotEmpty(orcid)) {
					OrcidProfile orcidProfile = orcidProfileManager
							.retrieveOrcidProfile(orcid);
					if (orcidProfile != null) {
						orcidProfile.setPassword(password);
						orcidProfileManager
								.updatePasswordInformation(orcidProfile);
					} else {
	...
	public @ResponseBody String removeSecurityQuestion(
			HttpServletRequest request, @RequestBody String orcidOrEmail) {
		if (StringUtils.isNotBlank(orcidOrEmail))
			orcidOrEmail = orcidOrEmail.trim();
		boolean isOrcid = matchesOrcidPattern(orcidOrEmail);
		String orcid = null;
	...
	public @ResponseBody Map<String, String> adminSwitchUser(
			@ModelAttribute("orcidOrEmail") String orcidOrEmail,
		...
		if (StringUtils.isNotEmpty(orcid)) {
			if (profileEntityManager.orcidExists(orcid)) {
				...
				if (!profileEntityManager.isProfileClaimed(orcid)) {
					mapOrcid.put(
							"errorMessg",
							new StringBuffer("Account for user ")
									.append(orcidOrEmail)
									.append(" is unclaimed.").toString());
				} else if (profileEntityManager.isDeactivated(orcid)) {
					mapOrcid.put(
							"errorMessg",
							new StringBuffer("Account for user ")
									.append(orcidOrEmail)
									.append(" is deactivated.").toString());
				} else if (profileEntityManager.isLocked(orcid)) {
					mapOrcid.put(
							"errorMessg",
							new StringBuffer("Account for user ")
									.append(orcidOrEmail).append(" is locked.")
									.toString());
				} else {
	...
	public @ResponseBody String adminVerifyEmail(
	...
	public @ResponseBody AdminDelegatesRequest startDelegationProcess(
			@RequestBody AdminDelegatesRequest request) {
		// Clear errors
		...
		if (!trustedIsOrcid) {
			if (emailManager.emailExists(trusted)) {
				...
				request.getTrusted()
						.getErrors()
						.add(getMessage(
								"admin.delegate.error.invalid_orcid_or_email",
								request.getTrusted().getValue()));
				haveErrors = true;
			}
		}
	...
	public @ResponseBody boolean checkClaimedStatus(
			@RequestParam("orcidOrEmail") String orcidOrEmail) {
		boolean isOrcid = matchesOrcidPattern(orcidOrEmail);
		String orcid = null;
		// If it is not an orcid, check the value from the emails table
		if (!isOrcid) {
	...
	public @ResponseBody ProfileDetails checkAccountToLock(
			@RequestBody String orcidOrEmail) {
		...
		if (PojoUtil.isEmpty(orcid)) {
			...
			result.getErrors().add(
					getMessage("admin.lock_profile.error.not_found",
							orcidOrEmail));
			return result;
		}
		...
		if (profile.isAccountNonLocked() == false) {
			...
			result.setErrors(new ArrayList<String>());
			result.getErrors()
					.add(getMessage("admin.lock_profile.error.already_locked",
							orcid));
			return result;
		}
	...
	public @ResponseBody ProfileDetails checkAccountToUnlock(
			@RequestBody String orcidOrEmail) {
		String orcid = getOrcidFromParam(orcidOrEmail);
		...
		if (profile.isAccountNonLocked()) {
			...
			result.setErrors(new ArrayList<String>());
			result.getErrors().add(
					getMessage("admin.unlock_profile.error.non_locked", orcid));
			return result;
		}
	...
	public @ResponseBody String lockAccount(@RequestBody String orcid) {
		...
		return getMessage("admin.lock_profile.error.couldnt_lock_account",
				orcid);
	}
	...
	public @ResponseBody String unlockAccount(@RequestBody String orcid) {
		...
		return getMessage("admin.unlock_profile.error.couldnt_unlock_account",
				orcid);
	}
}
